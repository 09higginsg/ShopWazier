...
class CartController {
  ...
  start(req, res) {
    ...
    const cart = CartService.startCart(session_id);

    // Send success response
    return res.json({ message: "cart started", session_id, cart });
  }
  ...
  async add(req, res) {
    ...
    const { session_id, barcode, qty, store_id } = req.body || {};

    // Check if required values are missing
    if (!session_id || !barcode || !store_id) {
      
      // Send error if missing
      return res.status(400).json({ error: "session_id, barcode and store_id required" });
    }

    // Call service to add item
    const result = await CartService.addItem(session_id, barcode, qty, store_id);
  ...
  async view(req, res) {
    ...
    const cart = await CartService.getCart(session_id, store_id);

    // If cart does not exist
    if (!cart) return res.status(404).json({ error: "cart not found" });

    // Get total price from service
    const total = await CartService.getTotal(session_id, store_id);
